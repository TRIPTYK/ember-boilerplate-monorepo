...
test("RefreshRoute returns new tokens on valid refresh token", async () => {
    const refreshToken = module.generateRefreshToken("some-user-id");
    await module.storeRefreshToken("some-user-id", refreshToken);
...
test("RefreshRoute returns JSON:API error when token is revoked", async () => {
    const refreshToken = module.generateRefreshToken("some-user-id");
    await module.storeRefreshToken("some-user-id", refreshToken, { revoked: true });
...
test("RefreshRoute returns JSON:API error when token is expired in database", async () => {
    const refreshToken = module.generateRefreshToken("some-user-id");
    await module.storeRefreshToken("some-user-id", refreshToken, { expired: true });
